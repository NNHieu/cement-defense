...
then
    command="python sig_attack.py --trainset_portion $portion --epochs $epochs --poisoning_rate $prate --model_name $model_name --lr $lr --optimizer_name $optimizer_name --seed $seed"
...
get_running_jobs_count() {
    tmux list-sessions 2>/dev/null | grep -c "^jobsig_"
}
